...
if __name__ == '__main__':
	...
	data_path = '/atlas_scratch/jschroder/Calibration_AR4_AR5/2ndrun/'
	...
	visual = os.path.join( data_path , 'Plots' )
...
#create a .p
	def get_filename(data_path , model):
		json_fn = os.path.join(data_path , 'JSON' , 'ALF_' + model + '.json')
		return json_fn
	...
	# data3 = {Scenario( get_filename(data_path , model), model, model, model , coldict[model]).__dict__['total_area_burned']['Boreal'].ix[1950:2100] for model in models[0:2]}

	def dummy(model):
		obj = Scenario( get_filename(data_path , model), model, model, model , coldict[model])
		df = defaultdict( lambda: defaultdict( lambda: defaultdict ) )
		df[model] = {metric : obj.__dict__[metric]['Boreal'] for metric in metrics}
		return df

	pool = mp.Pool( len(models) )

	tab = pool.map( dummy,models)
	pool.close()

	tt = dict([(key,d[key]) for d in tab for key in d])
	pic = os.path.join(visual, 'RCP60-Sresab1.p')
	pickle.dump( tt, open( pic, "wb" ) )  
