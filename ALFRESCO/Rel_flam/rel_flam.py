...
def sum_firescars2( firescar_list, ncores ):
	...
	repsums = pool.map( run, firescar_groups[0:100] )
	repsums1 = pool.map( run, firescar_groups[101:200] )
	# repsums = [ pool.map( lambda fn: np.sum( [ rasterio.open( fn ).read( 3 ), group ), axis=0 ) for group in firescar_groups ]
	pool.close()
	sum_arr = np.sum( [repsums + repsums1], axis=0 )
	return sum_arr
...
def relative_flammability( args ):
	...
	print 'done with meta update'
	if crs:
		meta.update( crs=crs )

	output_filename = os.path.join( output_path, 'alfresco_relative_flammability_' + args.model + '_' + args.scenario + '_' + str(1901) + '_' + str(2100) + '.tif' )
