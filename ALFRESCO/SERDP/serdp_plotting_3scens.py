...
def decade_plot( model , graph_variable, year_range=(1950,2100)):
	...
	green_patch = mpatches.Patch([], [], linewidth=1.2, color= colors_list[1] , label='With current FMO' )
	red_patch = mpatches.Patch([], [], linewidth=1.2, color=colors_list[2], label='With modified FMO' )
	gren_patch = mpatches.Patch([], [], linewidth=1.2, color=colors_list[3], label='Historical' )
	plt.legend(handles = [blu_patch , green_patch , red_patch, gren_patch],loc='best',ncol=1, shadow=True, fancybox=True)
...
def compare_area_burned(model , year_range=(1950,2100)):
	...
	# tmp = tmp.value[['scenario_3', 'scenario_2']]
	colors = [(0.2980392156862745, 0.4470588235294118, 0.6901960784313725),(0.7686274509803922, 0.3058823529411765, 0.3215686274509804),
	...
	#checking if colors_list list work
	obs_domain.plot( legend=False, linestyle= '--', color=colors_list[3], grid=False, label= "observed" ,lw = 1.1)
	...
	for cond, cond_df in df.groupby("condition"):
		...
		i = int(cond[-1])-1
		ax.fill_between(x, low, high, alpha=.1, color=colors_list[i])


	# build and display legend
	
	blu_patch = mlines.Line2D([], [], linewidth=1.2, color=colors[0], label='Without fire management' )
	green_patch = mlines.Line2D([], [], linewidth=1.2, color= colors[2] , label='With current FMO' )
	red_patch = mlines.Line2D([], [], linewidth=1.2, color=colors[1], label='With modified FMO' )
	ired_patch = mlines.Line2D([], [], ls='--', linewidth=1, color=colors_list[3], label='Historical' )
	#Setting legend
	plt.legend(handles = [blu_patch,green_patch,red_patch,ired_patch,],loc="best",ncol=1, shadow=True, fancybox=True)
	sns.despine()

	output_filename = os.path.join( output_path, '_'.join([ 'alfresco_annual_areaburned_compared_lines', model ,domain_name , str(begin), str(end),model ]) + '.png' )
	print "Writing %s to disk" %output_filename
	plt.savefig( output_filename )
	pdf.savefig()
	plt.close()
...
def compare_vegcounts(model ,veg_name_dict, year_range=(1950,2100)):
	...
	for veg_num, veg_name in veg_name_dict.iteritems():
		...
		plot_title = "Annual %s Coverage %s-%s \n ALFRESCO, %s, RCP 8.5"\
			% ( veg_name, str(begin), str(end),model)
		...
		green_patch = mlines.Line2D([], [], linewidth=1.2, color= colors_list[1] , label='With current FMO' )
		red_patch = mlines.Line2D([], [], linewidth=1.2, color=colors_list[2], label='With modified FMO' )
		...
		# else :
		plt.legend(handles = [blu_patch,green_patch,red_patch],loc="best",ncol=1, shadow=True, fancybox=True)
...
def compare_firesize( model , year_range=(1950,2100) , buff=False):
	...
	if buff==True :
		...
		output_filename = os.path.join( output_path, '_'.join([ 'alfresco_avgfiresize_compared',domain_name , model , str(begin), str(end),model ]) + '.png' )
	print "Writing %s to disk" %output_filename
	sns.despine()
	plt.savefig( output_filename )
	pdf.savefig()
...
def compare_cab_vs_fs(model , year_range=(1950,2100)):
	...
	fig.suptitle('Cumulative Area Burned vs. Fire Sizes %d-%d \n ALFRESCO, %s, RCP 8.5' \
						% ( begin, end, model))
	plt.legend( handles=[blu_patch,green_patch,red_patch], frameon=False, loc=0 )
...
if __name__ == '__main__':
	...
	# data_path = '/workspace/Shared/Users/jschroder/ALFRESCO_SERDP/Data/SERDP_Boreal_run/'
	data_path = '/workspace/Shared/Users/jschroder/ALFRESCO_SERDP/Data/SERPD_run'
	domain_num = 1
	# domain_name = 'Boreal'
	...
	visu = 'Visual_outputs_3scenarios'
	...
	colors_list = sns.color_palette('deep',7)

	
	scenarios = [ 'scenario_1', 'scenario_2', 'scenario_3']
	year_range = (1950,2100)
	# This part was added in order to deal with the new Post processing, here it take care of renaming the files
	# to make it easier to plot, NoFMO = scenario_1, AltFMO = scenario_3 and 'nothing' = scenario_1
	...
	for model in models :
		...
		pdf_output =  os.path.join( visu, '_'.join([ domain_name, model,'plots' ]) + '.pdf' )

		with PdfPages(pdf_output) as pdf:

			for metric in metrics[:-2]:
				decade_plot(model , metric, year_range)

			compare_area_burned(model,year_range)
			compare_vegcounts(model, veg_name_dict,year_range)
			compare_firesize( model,year_range)
			compare_numberoffires( model,year_range)
			compare_cab_vs_fs(model,year_range)
