...
def decade_plot( model , graph_variable, year_range=(1950,2100)):
	...
	green_patch = mpatches.Patch([], [], linewidth=1.2, color= colors_list[0] , label='Current FMPO' )
	red_patch = mpatches.Patch([], [], linewidth=1.2, color=colors_list[1], label='Alternative FMPO' )
	...
	output_filename = os.path.join( output_path, '_'.join([ 'alfresco', domain_name,graph_variable,'decade', model , str(begin), str(end),model ]) + '.png' )
...
def compare_area_burned(model , year_range=(1950,2100)):
	...
	colors = [colors_list[1],colors_list[0],colors_list[2]]
	...
	green_patch = mlines.Line2D([], [], linewidth=1.2, color= colors[1] , label='Current FMPO' )
	red_patch = mlines.Line2D([], [], linewidth=1.2, color=colors[0], label='Alternative FMPO' )
...
def compare_vegcounts(model ,veg_name_dict, year_range=(1950,2100)):
	...
	for veg_num, veg_name in veg_name_dict.iteritems():
		...
		for cond, cond_df in df.groupby("condition"):
			low = cond_df.groupby("date").value.apply(np.percentile, 5)
			high = cond_df.groupby("date").value.apply(np.percentile, 95)
			if cond == 'scenario_2' : i = 0
			elif cond == 'scenario_3' : i =1
		...
		green_patch = mlines.Line2D([], [], linewidth=1.2, color= colors_list[0] , label='Current FMPO' )
		red_patch = mlines.Line2D([], [], linewidth=1.2, color=colors_list[1], label='Alternative FMPO' )
...
if __name__ == '__main__':
	...
	visu = 'Visual_outputs_2scenarios'
	...
	# to make it easier to plot, NoFMPO = scenario_1, AltFMPO = scenario_3 and 'nothing' = scenario_1
	for metric in metrics :
		...
		for i in l :
			if os.path.splitext( os.path.basename( i ) )[0].split( '_' )[-3] == 'rcp85' :
				...
				os.rename(i, a)
			elif os.path.splitext( os.path.basename( i ) )[0].split( '_' )[-3] == 'NoFMPO':
				a = i.replace('NoFMPO','scenario_1')
				os.rename(i, a)
			elif os.path.splitext( os.path.basename( i ) )[0].split( '_' )[-3] == 'AltFMPO':
				a = i.replace('AltFMPO','scenario_3')
				os.rename(i, a)
			else : pass
